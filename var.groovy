...
foreach (\$file in \$rdlFiles) {
    ...
    if (\$existing) {
        ...
        if (\$cfg.AllowReplace -eq 'true') {
            ...
            Apply-OdbcConnectionReplace -fullPath \$fullPath -findString \$cfg.FindString -replaceString \$cfg.ReplaceString | Out-Null
        ...
        \$importId = (Import-RdlFile -groupId \$cfg.GroupId -fileName \$fileName -fullPath \$fullPath -mode \$cfg.GroupId -headers \$headers)
        Write-Host "Import ID: \$importId"
